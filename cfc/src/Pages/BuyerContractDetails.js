...
const BuyerContractDetails = () => {
  ...
  return (
    <div className="p-6 bg-green-50 min-h-screen max-w-4xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold text-green-700">Contract Details (Buyer View)</h2>
      <div className="bg-white rounded-lg shadow p-6 space-y-4">
        <p><strong>Crop:</strong> {contract.crop}</p>
        <p><strong>Farmer Email:</strong> {contract.farmerEmail}</p>
      ...
      <div className="flex justify-center gap-4 mt-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-white ${buyerHasDone ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}
        >
          {buyerHasDone ? 'You marked Done' : (actionLoading ? 'Processing...' : 'Negotiation Done')}
        </button>
      ...
      {overallStatus === 'Negotiation Done' && farmerHasDone && buyerHasDone && (
        <p className="text-green-700 mt-3 text-center font-semibold">Both parties completed negotiation. Waiting for farmer's final contract.</p>
      )}
    </div>
  );
};
